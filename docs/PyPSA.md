...
Each component has a set of attributes with data types, default values, and descriptions for each attribute. For instance, attributes for capacity, efficiency, costs, and the buses to which components are attached.
...
By default, the network is optimised for a single deterministic scenario (`n.has_scenarios` is `False`). If scenarios are defined, the network is optimised for multiple scenarios in form of a risk-neutral two-stage stochastic programming framework.
...
**Static data** is stored in a `pandas.DataFrame`, which is an attribute of the `pypsa.Network`, with names that follow the component names. For instance, `n.buses`. In this `pandas.DataFrame`, the index corresponds to the unique string names of the components, while the columns correspond to the components' static attributes.

**Time-varying data** is stored in a dictionary of `pandas.DataFrame` objects, which is an attribute of the `pypsa.Network`, with names that follow the component names with a `_t` suffix. For instance, `n.buses_t`.
...
Attributes that can be time-varying are marked as "series" in the component documentation.
...
The component documentation shows for each attribute whether it is an input (which the user specifies) or output (which is computed by PyPSA). Inputs can be either "required" or "optional". Optional inputs are assigned a sensible default value if the user gives no input.
...
| CO₂ emissions | tonnes of CO₂ per MWh~thermal~ of energy carrier |
...
The units of buses can also refer to non-electric carriers, such as tonnes of hydrogen (t~H2~). In this case, the units of energy would be t~H2~ with units of power of t~H2~/h. Unit conversions are not applied automatically handled, but must be encoded through the efficiencies.
